class ExpaOpportunity < ActiveRecord::Base
  ...
  def update_from_expa(data)
    ...
    self.xp_programmes = (data.programmes.nil?) ? {} : data.programmes.to_json.to_s 
    ...
    self.xp_skills = (data.skills.nil?) ? {} : data.skills 
    self.xp_backgrounds = (data.backgrounds.nil?) ? {} : data.backgrounds
    self.xp_languages = (data.languages.nil?) ? {} : data.languages
    self.xp_issues = (data.issues.nil?) ? {} : data.issues
    self.xp_work_fields = (data.work_fields.nil?) ? {} : data.work_fields
    self.xp_study_levels = (data.study_levels.nil?) ? {} : data.study_levels
    self.xp_prefered_locations = (data.prefered_locations.nil?) ? {} : data.prefered_locations
    self.xp_role_info = (data.role_info.nil?) ? {} : data.role_info
    self.xp_logistic_info = (data.logistic_info.nil?) ? {} : data.logistic_info
    self.xp_legal_info = (data.legal_info.nil?) ? {} : data.legal_info
    self.xp_specifics_info = (data.specifics_info.nil?) ? {} : data.specifics_info
    self.xp_department = data.department unless data.department.nil?
    self.xp_tm_details = (data.tm_details.nil?) ? {} : data.tm_details
